...
class MongoCollectionServer extends AbstractMongoServer {
  _addGetAllRoute () {
    this._addRoute('getAll', 'get', '/', (req, res) => {
      this._useCollection(collection => {
        return Promise.resolve(collection.find({}))
  ...
  _addSearchRoute () {
    this._addRoute('search', 'get', '/search', (req, res) => {
      this._useCollection(collection => {
        return Promise.resolve(collection.find(req.query))
          .then(data => res.status(200).json(data.map(datum => new this._model(datum).toJson())))
      }, res)
    })
  }
    ...
    this._addRoute('create', 'post', '/', (req, res) => {
      ...
      this._useCollection(collection => {
        return Promise.resolve(entry.validate())
          .then(() => entry.sanitize())
          .then(sanitizedEntry => collection.insertOne(sanitizedEntry))
          .then(({ insertedId }) => {
  ...
  _addGetRoute () {
    this._addRoute('get', 'get', '/:id', (req, res) => {
      this._useCollection(collection => {
        return Promise.resolve(collection.findOne({ _id: new ObjectId(req.params.id) }))
          .then(data => {
            if (data) {
          ...
          .then(data => {
            if (data) {
              ...
              return entry.validate()
                .then(() => entry.sanitize())
                .then(sanitizedEntry => collection.updateOne({ _id: new ObjectId(req.params.id) }, { $set: sanitizedEntry }))
                .then(() => {
  ...
  _addDeleteRoute () {
    this._addRoute('delete', 'delete', '/:id', (req, res) => {
      this._useCollection(collection => {
        return Promise.resolve(collection.deleteOne({ _id: new ObjectId(req.params.id) }))
          .then(({ deletedCount }) => {
            if (deletedCount === 0) {
              res.sendStatus(404)
            } else {
